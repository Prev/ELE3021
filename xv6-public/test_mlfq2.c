    ...
    while (1) {
        ...
        if (i == YIELD_PERIOD) {
            ...
            if (curr_tick - start_tick > LIFETIME) {
              //total = cnt_level[0] + cnt_level[1] + cnt_level[2];
            //if (i > LIFETIME) {
                printf(1, "MLFQ(%s), lev[0]: %d, lev[1]: %d, lev[2]: %d, total:%d\n",
                        do_yield==0 ? "compute" : "yield",
                        cnt_level[0], cnt_level[1], cnt_level[2], cnt);
              break;
            }
